...
def train_pre(hparams_train,hparams_prefix):
    early_stopping = EarlyStopping(monitor='val_loss', patience=3, mode='min')
    checkpoint_callback = ModelCheckpoint(
            dirpath='./log/pre_check',
            filename='{epoch}-{val_loss:.2f}-{other_metric:.2f}'
        )
    lr_callback = LearningRateMonitor(logging_interval="step")
    model = Bart_Prefix_Model(hparams_train,hparams_prefix)
    logger = TensorBoardLogger(save_dir=os.path.join(hparams_train['data_dir'],'prefix'),name=hparams_train['model']+'_log')

    trainer = pl.Trainer(max_epochs=hparams_train['max_epochs'], callbacks=[early_stopping,checkpoint_callback,lr_callback], logger=logger,
                     default_root_dir=os.path.join(hparams_train['data_dir'],hparams_train['model']+'_save'),
...
def train_t2t(hparams):
    ...
    trainer = pl.Trainer(max_epochs=hparams['max_epochs'], callbacks=[early_stopping,checkpoint_callback ,lr_callback], logger=logger,
                         #auto_lr_find=True,
...
if __name__ =="__main__":
    ...
    train_pre(hparams_prefix_train,prefix_args())
    #train_pre(datadir="./dataset/Wiki10-31K/",model=model,checkdir='prefix_check',output='prefix_save',
